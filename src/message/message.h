...
/*
	...
	---------------------------------------------------------------------------------------------------------------------------------------------------------------------
		...
		Т.е. модуль может опубликовать любое событие ТОЛЬКО КАК нагрузку к сообщению EventResponse ("публикация события"). В этом случае необходимо различать, что
		структура такой нагрузки включает в себя ТОЛЬКО данные события, в то время как приходящее сообщение типа "Событие" (Event) - содержит расшифровку события в своей нагрузке.
	...
	---------------------------------------------------------------------------------------------------------------------------------------------------------------------
		...
		если модуль хочет опубликовать это событие, то он должен отослать его в ответ на запрос EventRequest как нагрузку сообщения EventResponse, 
			...
			при этом [нагрузка, зависящая от события]:
